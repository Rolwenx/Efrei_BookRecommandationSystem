...
"""Function to create the scoring matrix that will be initialized the first time that the program will be launched
...
""" This function overwrites the scoring matrix and re-writes it with the list put as parameters
    PARAMETERS:
...
def Rate_A_Book():
    ...
    does_reader_exist = readersfunctions.Check_If_User_Exists_And_Return_Rank(searched_reader)
...
def same_book_read_and_rated(user1, user2):
    ...
    with open("booksread.txt", "r") as read_books, open("scoring_matrix.txt", "r") as rates_of_books:
        ...
        rate_b_user1 = cut_space_from_list(
            transform_str_list(rate_b[user1]))  # We delete all spaces from the variable and transform it into a list
        rate_b_user2 = cut_space_from_list(transform_str_list(rate_b[user2]))  # For example ['4','3','0','0','5','0']
        list_return = []
        for i in range(len(rate_b_user2)):
            if int(rate_b_user2[i]) != 0 or int(
                    rate_b_user1[i]) != 0:  # We check if at least one of the two user read the book
